...
export const Canvas: React.FC<CanvasProps> = ({
  ...
  }, [activeTool, isPanning, isDragging, panOffset, onCreateTodo, onClearSelection]);
  ...
  const handleTodoDragStart = useCallback((todoId: string, e: React.MouseEvent) => {
    if (activeTool === 'cursor') {
      startDrag([todoId], e, todoPositions);
    } else if (activeTool === 'move') {
      const idsToMove = selectedIds.includes(todoId) && selectedIds.length > 1 
        ? selectedIds 
        : [todoId];
  ...
  const handleTodoSelect = useCallback((todoId: string) => {
    if (activeTool === 'move') {

...
export const SubtaskItem: React.FC<SubtaskItemProps> = ({
  ...
  const handleKeyDown = useCallback((e: React.KeyboardEvent) => {
    switch (e.key) {
      case 'Enter':
        e.preventDefault();
        if (e.shiftKey) {
          // Shift+Enter = Nova subtarefa indentada
          onCreate(index, subtask.level + 1);
        } else {
          // Enter = Nova subtarefa no mesmo nível
  ...
  }, [index, subtask.level, subtask.id, text, onCreate, onIndent, onOutdent, onRemove]);

  // Calcular indentação visual
  const indentStyle = {
    marginLeft: `${subtask.level * 20}px` // 20px por nível
  };

  // Cor da linha de indentação
  ...
  return (
    <div 
      className={`flex items-center gap-2 py-1 ${subtask.level > 0 ? 'border-l-2 ' + getIndentColor(subtask.level - 1) : ''}`}
      style={indentStyle}
    >
      {/* Checkbox */}
      <button
      ...
      <input
        ...
        placeholder={subtask.level === 0 ? "Nova tarefa..." : "Nova subtarefa..."}
